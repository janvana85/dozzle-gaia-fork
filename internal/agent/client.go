...
func (c *Client) UpdateNotificationConfig(ctx context.Context, subscriptions []types.SubscriptionConfig, dispatchers []types.DispatcherConfig) error {
	...
	for i, sub := range subscriptions {
		pbSubs[i] = &pb.NotificationSubscription{
			...
			QuietPriority:             int32(sub.QuietPriority),
			HoldDuringQuiet:           sub.HoldDuringQuiet,
			HoldClearWindow:           int32(sub.HoldClearWindow),
			BurstCount:                int32(sub.BurstCount),
			BurstWindow:               int32(sub.BurstWindow),
			BurstPriority:             int32(sub.BurstPriority),
			WatchdogPattern:           sub.WatchdogPattern,
			WatchdogWindow:            int32(sub.WatchdogWindow),
			WatchdogCooldown:          int32(sub.WatchdogCooldown),
			...
			RestartLoopStateWindow:    int32(sub.RestartLoopStateWindow),
			RestartLoopEventCount:     int32(sub.RestartLoopEventCount),
			RestartLoopEventWindow:    int32(sub.RestartLoopEventWindow),
			RestartLoopCooldown:       int32(sub.RestartLoopCooldown),
			RestartLoopTriggerMessage: sub.RestartLoopTriggerMessage,
			QuietStackThreshold:       int32(sub.QuietStackThreshold),
			QuietStackWindow:          int32(sub.QuietStackWindow),
			...
			UniqueWindow:              int32(sub.UniqueWindow),
			UniqueThreshold:           int32(sub.UniqueThreshold),
		}
	}
